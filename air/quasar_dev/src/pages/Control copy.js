...
const MqttPage = () => {
        ...
        html2canvas(chartElement, { scale: 2 })
            .then((canvas) => {
                canvas.toBlob((blob) => {
                    ...
                    // Convert chart data to JSON blob with mapped keys
                    const mappedChartData = chartData.map((dataPoint) => ({
                        time: dataPoint.time,
                        timestamp: dataPoint.timestamp,
                        [tempLabels[1]]: dataPoint.temp1 ?? 0,
                        [tempLabels[2]]: dataPoint.temp2 ?? 0,
                        [tempLabels[3]]: dataPoint.temp3 ?? 0,
                        [tempLabels[4]]: dataPoint.temp4 ?? 0,
    ...
    }, [adjustedData]); // ✅ Ensure it updates correctly
                ...
                <div className="w-[35%] bg-gray-100 p-4 rounded shadow-md border border-gray-400">
                    ...
                    <div className="flex gap-2 mt-2">
                        ...
                        <div className="flex flex-col items-center mt-10 gap-2 w-full">
                            ...
                            <div className="bg-white p-4  mt-10 rounded shadow border border-gray-300 w-5/6 text-center">
                                <h3 className="text-lg font-semibold text-gray-800">加熱溫度</h3>

                                <div className="grid grid-cols-2 gap-2 mt-2">
                                    {[1, 2, 3, 4].map((num) => {
                                        const tempValue = heaterTemp?.[`temp${num}`] ?? 0;
                                        const isError = tempValue < 15;

                                        return (
                                            <div
                                                key={num}
                                                className="flex flex-col items-center text-center bg-gray-100 p-4 rounded shadow-md border border-gray-300 w-35"
                                            >
