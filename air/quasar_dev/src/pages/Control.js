...
const MqttPage = () => {
    ...
    // Utility function to get initial test mode from cookies
    const getInitialTestMode = () => {
        const stored = Cookies.get("TEST_MODE");
        return stored ? stored : "auto"; // Default to "auto"
    };

    // State variables
    const [testMode, setTestMode] = useState(getInitialTestMode());

    // const [testMode, setTestMode] = useState("auto"); // Default: Auto
    // const [manualTemp, setManualTemp] = useState(300); // Default 300°C
    // const [manualDelay, setManualDelay] = useState(30); // Default 30s
    const [manualTesting, setManualTesting] = useState(false); // Track manual test state


    // Save test mode to cookies whenever it changes
    useEffect(() => {
        Cookies.set("TEST_MODE", testMode, { expires: 365 });
    }, [testMode]);


    // Utility functions to get initial values from cookies
    const getInitialManualTemp = () => {
        const stored = Cookies.get("MANUAL_TEMP");
        return stored ? parseInt(stored, 10) : 300; // Default to 300°C
    };

    const getInitialManualDelay = () => {
        const stored = Cookies.get("MANUAL_DELAY");
        return stored ? parseInt(stored, 10) : 30; // Default to 30s
    };

    // In your component's state:
    const [manualTemp, setManualTemp] = useState(getInitialManualTemp());
    const [manualDelay, setManualDelay] = useState(getInitialManualDelay());



    // Save to cookies when the values change:
    useEffect(() => {
        Cookies.set("MANUAL_TEMP", manualTemp, { expires: 365 });
    }, [manualTemp]);

    useEffect(() => {
        Cookies.set("MANUAL_DELAY", manualDelay, { expires: 365 });
    }, [manualDelay]);
    ...
    const TOPIC_HEATER_MANUAL_CMD = "autotest/manual1"; // Manual test topic

    const sendManualHeatCommand = () => {
        if (mqttClient.current) {
            setManualTesting(true); // Disable button during test
            const payload = JSON.stringify({
                temp: manualTemp,
                time: manualDelay,
            });
            mqttClient.current.publish(TOPIC_HEATER_MANUAL_CMD, payload);
            console.log(`Published manual test:`, payload);
    ...
    let prevValid = { temp2: null, temp3: null, temp4: null };

    const adjustedData = chartData.map((point) => {
        const t1 = (point.temp1 ?? 0) + tempOffset;
        let t2 = (point.temp2 ?? 0) + tempOffset;
        let t3 = (point.temp3 ?? 0) + tempOffset;
        let t4 = (point.temp4 ?? 0) + tempOffset;

        // Validate temp2: if t2 is over 1000 or differs from t1 by more than 220,
        // use previous valid value (if available) or fallback to t1.
        if (t2 > 1000 || Math.abs(t2 - t1) > 120) {
            t2 = prevValid.temp2 !== null ? prevValid.temp2 : t1;
        } else {
            prevValid.temp2 = t2;
        }
        // Validate temp3
        if (t3 > 1000 || Math.abs(t3 - t1) > 120) {
            t3 = prevValid.temp3 !== null ? prevValid.temp3 : t1;
        } else {
            prevValid.temp3 = t3;
        }
        // Validate temp4
        if (t4 > 1000 || Math.abs(t4 - t1) > 120) {
            t4 = prevValid.temp4 !== null ? prevValid.temp4 : t1;
        } else {
            prevValid.temp4 = t4;
        }
        ...
        return {
            ...point,
            time: point.time - minTime, // adjust time
            temp1: t1,
            temp2: t2,
            temp3: t3,
            temp4: t4,
    ...
    // Define a disable condition for manual test button:
    const manualDisabled =
        isDrawing ||
        drawingCompleted ||
        temp1Error ||
        relayDataStatus !== "繼電器偵測正常" ||
        heaterDataStatus !== "溫度偵測正常" ||
        manualTesting; // also disable if already testing
                ...
                <div className="w-[35%] bg-gray-100 p-4 rounded shadow-md border border-gray-400">
                    ...
                    <div className="relative w-full mb-4">
                        ...
                        <div className="absolute right-0 top-1/2 transform -translate-y-1/2 flex items-center space-x-2">
                            <span className="text-xs text-gray-700">自動</span>
                            <label className="flex items-center cursor-pointer">
                                <input
                                    type="checkbox"
                                    checked={testMode === "manual"}
                                    onChange={() => setTestMode(testMode === "auto" ? "manual" : "auto")}
                                    className="hidden"
                                />
                                <div className={`w-10 h-5 flex items-center rounded-full p-1 duration-300 ease-in-out ${testMode === "manual" ? "bg-blue-500" : "bg-gray-400"}`}>
                                    <div className={`bg-white w-4 h-4 rounded-full shadow-md transform duration-300 ease-in-out ${testMode === "manual" ? "translate-x-5" : "translate-x-0"}`}></div>
                                </div>
                            </label>
                            <span className="text-xs text-gray-700">手動</span>
                        </div>
                    ...
                    {/* Conditional Rendering for Auto / Manual */}
                    {testMode === "auto" ? (
                        // Auto Test Mode (Default)
                        ...
                        <div className="flex gap-2 items-center">
                            <div className="flex items-center space-x-4">
                                ...
                                <div className="flex flex-col items-center">
                                    <span className="text-sm font-semibold text-gray-700">溫度 (°C)</span>
                                    <input
                                        type="number"
                                        value={manualTemp}
                                        onChange={(e) => setManualTemp(Number(e.target.value))}
                                        className="border border-gray-400 rounded p-2 w-24 text-center text-lg text-purple-600 font-bold"
                                        disabled={isDrawing || drawingCompleted || temp1Error || relayDataStatus !== "繼電器偵測正常" || heaterDataStatus !== "溫度偵測正常"}
                                    />
                                </div>
                                ...
                                {/* Delay Time Input */}
                                <div className="flex flex-col items-center">
                                    <span className="text-sm font-semibold text-gray-700">維持時間 (秒)</span>
                                    <input
                                        type="number"
                                        value={manualDelay}
                                        onChange={(e) => setManualDelay(Number(e.target.value))}
                                        className="border border-gray-400 rounded p-2 w-24 text-center text-lg text-purple-600 font-bold"
                                        disabled={isDrawing || drawingCompleted || temp1Error || relayDataStatus !== "繼電器偵測正常" || heaterDataStatus !== "溫度偵測正常"}
                                    />
                                </div>
                                ...
                                <button
                                    ...
                                    className={`w-10 h-16 px-1 py-1 text-xs rounded shadow-md transition-colors ${isDrawing ||
                                        drawingCompleted ||
                                        temp1Error ||
                                        relayDataStatus !== "繼電器偵測正常" ||
                                        heaterDataStatus !== "溫度偵測正常"
                            ...
                            <button
                                className={`flex-1 px-4 py-2 h-16 rounded text-xl text-white text-base font-medium shadow-md transition-colors ${manualDisabled ? "bg-gray-400 cursor-not-allowed" : "bg-purple-600 hover:bg-purple-700"
                                    }`}
                                onClick={sendManualHeatCommand}
