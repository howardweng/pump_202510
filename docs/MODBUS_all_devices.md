...
|------|----------|------------|---------|
| DC 電表 | 1 | `01 03 10 00 00 04 40 C9` | 4 個寄存器 (8 bytes) |
| AC110V 電表 | 2 | `02 03 10 00 00 04 40 FA` | 4 個寄存器 (8 bytes) |
| AC220V 電表 | 3 | `03 03 10 00 00 04 41 2B` | 4 個寄存器 (8 bytes) |
| AC220V 3P 電表 | 4 | `04 03 10 00 00 11 81 53` | **17 個寄存器 (34 bytes)** ⭐ |
...
| Byte 位置 | 參數名稱 | 換算公式 | 單位 | 說明 |
|-----------|---------|---------|------|------|
| Byte 4-5 | A 相電壓有效值 | Hex → Dec ÷ 100 | V | Phase A Voltage |
| Byte 6-7 | B 相電壓有效值 | Hex → Dec ÷ 100 | V | Phase B Voltage |
| Byte 8-9 | C 相電壓有效值 | Hex → Dec ÷ 100 | V | Phase C Voltage |
| Byte 10-11 | A 相電流有效值 | Hex → Dec ÷ 1000 | A | Phase A Current |
| Byte 12-13 | B 相電流有效值 | Hex → Dec ÷ 1000 | A | Phase B Current |
| Byte 14-15 | C 相電流有效值 | Hex → Dec ÷ 1000 | A | Phase C Current |
| Byte 16-17 | 0 相電流有效值 (漏電流) | Hex → Dec ÷ 1000 | A | Neutral/Leakage Current |
| Byte 18-19 | A 相有功功率 | Hex → Dec ÷ 10000 | kW | Phase A Active Power |
| Byte 20-21 | B 相有功功率 | Hex → Dec ÷ 10000 | kW | Phase B Active Power |
| Byte 22-23 | C 相有功功率 | Hex → Dec ÷ 10000 | kW | Phase C Active Power |
| Byte 24-25 | 合相有功功率 | **Signed Int32** ÷ 10000 | kW | **Total Active Power** |

**⚠️ 重要**:
- Byte 24-25 為 **有符號 32 位元整數** (Signed Int32)
- 三相電表回應長度為 **34 bytes**，遠大於單相電表的 8 bytes
- 提供每相獨立的電壓、電流、功率測量值
...
位元 4-5 ：A相电压有效值 需換算成 10進制 数据/100 单位V
位元 6-7 ：B相电压有效值 需換算成 10進制 数据/100 单位V
位元 8-9 ：C相电压有效值 需換算成 10進制 数据/100 单位V
位元 10-11 ：A相电流有效值 需換算成 10進制 数据/1000 单位A
位元 12-13 ：B相电流有效值 需換算成 10進制 数据/1000 单位A
位元 14-15 ：C相电流有效值 需換算成 10進制 数据/1000 单位A
位元 16-17 ：0相（漏电流）电流有效值 需換算成 10進制 数据/1000 单位A
位元 18-19 ：A相有功功率 需換算成 10進制 数据/10000 单位kW
位元 20-21 ：B相有功功率 需換算成 10進制 数据/10000 单位kW
位元 22-23 ：C相有功功率 需換算成 10進制 数据/10000 单位kW
位元 24-25 ：合相有功功率：有符号Int32，单位kW，结果=数据/10000


IO 返回 01 02 01 00 A1 88
        位元 4 返回的输入通道状态 bit 0-8
