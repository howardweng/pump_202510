...
function MainDashboard() {
  ...
  const [systemMessage, setSystemMessage] = useState(null); // { type: 'warning'|'emergency'|'reset'|'info', text: string }
  ...
  return (
    <div className="w-full px-4 py-2">
      {/* 左側配置 + 右側內容 */}
      <div className="grid grid-cols-10 gap-4 mb-3" style={{ minHeight: '600px' }}>
        {/* 左側：感應器狀態、訊息區域和配置 (20%) */}
        <div className="col-span-2 flex flex-col gap-4 h-full">
          {/* 感應器狀態 - 佔據30%高度 */}
          <div className="bg-white border-2 border-slate-300 rounded-lg p-3 shadow-md flex-[3] flex flex-col min-h-[180px]">
            <h3 className="text-sm font-bold text-slate-800 mb-2 pb-2 border-b border-slate-300 text-center">感應器狀態</h3>
          ...
          {/* 訊息區域 - 佔據30%高度 (flex: 3) */}
          <div className="bg-white border-2 border-slate-300 rounded-lg p-4 shadow-md flex-[3] flex flex-col min-h-[180px]">
            <h3 className="text-base font-bold text-slate-800 mb-3 pb-2 border-b border-slate-300 text-center">訊息</h3>
            <div className="flex-1 flex items-center justify-center">
              {systemMessage ? (
                <div className={`w-full p-6 rounded-lg border-2 ${
                  systemMessage.type === 'emergency' 
                    ? 'bg-red-100 border-red-500' 
                    : systemMessage.type === 'warning'
                    ? 'bg-yellow-100 border-yellow-500'
                    : systemMessage.type === 'reset'
                ...
                <div className="w-full p-6 rounded-lg bg-gray-50 border-2 border-gray-200 flex items-center justify-center h-full">
                  <p className="text-lg font-medium text-gray-500 text-center">無訊息</p>
                </div>
              )}
            </div>
          </div>

          {/* 當前測試配置 - 佔據剩餘空間 (40%, flex: 4) */}
          {testConfig.pumpModel ? (
            <div className="bg-blue-50 border-2 border-blue-300 rounded-lg p-3 shadow-md flex-[4] flex flex-col min-h-0">
              <h3 className="text-sm font-bold text-slate-800 mb-2 pb-1 border-b border-blue-300 text-center">當前測試配置</h3>
              <div className="space-y-1.5 text-xs flex-1">
                <div className="flex items-start">
                  <span className="text-slate-400 font-medium w-16 flex-shrink-0">型號:</span>
                  <span className="text-sm font-bold text-[#0066CC] flex-1 break-words">{testConfig.pumpModel}</span>
                </div>
                <div className="flex items-start">
                  <span className="text-slate-400 font-medium w-16 flex-shrink-0">測試模式:</span>
                  <span className="text-sm font-bold text-[#0066CC] flex-1">
                    {testConfig.testMode === 'vacuum' ? '真空幫浦' : testConfig.testMode === 'positive' ? '正壓幫浦' : '手動模式'}
                  </span>
                </div>
                <div className="flex items-start">
                  <span className="text-slate-400 font-medium w-16 flex-shrink-0">測試類型:</span>
                  <span className="text-sm font-bold text-[#0066CC] flex-1">
                    {testConfig.testType === 'pressure' ? '壓力測試' : '流量測試'}
                  </span>
                </div>
                <div className="flex items-start">
                  <span className="text-slate-400 font-medium w-16 flex-shrink-0">電源:</span>
                  <span className="text-sm font-bold text-[#0066CC] flex-1">{testConfig.powerSource || '-'}</span>
                </div>
                <div className="flex items-start">
                  <span className="text-slate-400 font-medium w-16 flex-shrink-0">額定功率:</span>
                  <span className="text-sm font-bold text-[#0066CC] flex-1">{testConfig.ratedPower} W</span>
                </div>
                <div className="flex items-start">
                  <span className="text-slate-400 font-medium w-16 flex-shrink-0">最大電流:</span>
                  <span className="text-sm font-bold text-[#0066CC] flex-1">{testConfig.maxCurrent} A</span>
                </div>
                <div className="flex items-start">
                  <span className="text-slate-400 font-medium w-16 flex-shrink-0">儲存目標:</span>
                  <span className="text-sm font-bold text-[#0066CC] flex-1">
                ...
                {testConfig.notes && (
                  <div className="flex items-start">
                    <span className="text-slate-400 font-medium w-16 flex-shrink-0">備註資料:</span>
                    <span className="text-sm font-bold text-[#0066CC] flex-1 break-words">{testConfig.notes}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="col-span-8 flex flex-col gap-4">
          ...
          {/* 圖表區域 */}
          <div ref={chartRef} className="bg-white rounded-lg shadow-lg p-4 border-2 border-slate-300 flex-1">
